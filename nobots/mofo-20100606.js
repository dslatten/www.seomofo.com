function lazyStyles(){
	if(document.getElementById('lazy_styles')){
		...
		styleArray.push(".custom .aff_link:hover{cursor:url('\/img\/cursors\/affiliate-link.cur'), pointer;}");
...
function subscribeLinks(){
	if(document.getElementById('sub_box')){
		...
		subArray.push("		<a target=\"_blank\" class=\"aff_link\" href=\"http:\/\/www.seomofo.com\/affiliate\/thesis\/\" onclick=\"_gaq.push(['_trackEvent', 'Affiliates', 'Thesis', 'Sidebar - Custom 125x125']);\"><span>Thesis<\/span><\/a>");
		subArray.push("	<\/div>");
		subArray.push("<\/div>");
...
function nofollowLinks(){
	var theURL, theAnchorText;
	var nfspan = document.getElementsByTagName('span');
	for (var i = 0; i<nfspan.length; i++){
		if (hasClass(nfspan[i], 'nofollow')){
			theAnchorText = nfspan[i].innerHTML;
			theURL = nfspan[i].title;
			nfspan[i].innerHTML = '<a href="http://' + theURL + '" class="' + nfspan[i].className + '" rel="nofollow">' + theAnchorText + '</a>';
		}
	}
}
