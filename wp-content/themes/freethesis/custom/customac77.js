...
function lazyLoadImages(){
	if((typeof mofoLazyPics != "undefined") && (typeof mofoImgs != "undefined")){
		for (var i = 0; i < mofoImgs.length; i++){
			mofoImgs[i].setAttribute("src", mofoLazyPics[i]);
		}
		for (var i = 0; i < mofoImgs.length; i++){
			mofoImgs[i].style.backgroundImage = "none";}
		}
	}
function subscribeLinks(){
	if(document.getElementById('sub_box')){
		var	subBox = document.getElementById('sub_box');
		var subFrame = "<iframe src=\"http:\/\/www.seomofo.com\/nobots\/iframes\/sidebar5.html\" frameborder=\"0\" scrolling=\"no\" width=\"127\" height=\"626\"><\/iframe>";
...
function tweetMemeWidget(){
	if((document.getElementById('tm_box')) && (typeof(noTweetMeme) == 'undefined')){
		if(typeof(mofoTwitterTitle)!='undefined'){
			var theTitle = encodeURIComponent(mofoTwitterTitle);
		}
		else{
...
function newTwitterButtonWidget(){
	if((document.getElementById('tw_box')) && (typeof(noTwitter) == 'undefined')){
		if(typeof(mofoTwitterTitle)!='undefined'){
			var theTitle = encodeURIComponent(mofoTwitterTitle);
		}
		else{
		...
		twArray.push("<iframe scrolling=\"no\" height=\"62\" frameborder=\"0\" width=\"58\" src=\"http:\/\/platform.twitter.com\/widgets\/tweet_button.html?count=vertical&amp;related=seomofo%3AWorld%27s%20Greatest%20SEO%20(verified)&amp;url=" + escape(document.URL) + "&amp;lang=en&amp;text=Great%20post%20by%20@SEOmofo%20%C2%BB%20" + theTitle + "%20%C2%BB\"" + "onload=\"document.getElementById('tw_loading').style.display='none';\"><\/iframe>");
